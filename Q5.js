d3.csv("data_ggsheet.csv").then(function(data) {
    ...
    const bars = chart.selectAll(".bar")
        ...
        .on("click", function () {
            const opacity = d3.select(this).attr("opacity") === "0.3" ? 1 : 0.3;
            bars.attr("opacity", 0.3);
            d3.select(this).attr("opacity", opacity === 1 ? 1 : 0.3);
        });
    ...
    chart.append("g")
        .call(d3.axisLeft(y).tickFormat(d => `${(d / 1_000_000).toFixed(0)}M`).ticks(15))
        .style("font-size", "11px");
